    ...
    internal class ClientPage
    {
        ...
        private void SearchProduct()
        {
            try
            {
                ...
                var table = new ConsoleTable("№", "Product Name", "Price", "Unit", "Residue", "Type", "Status");

                foreach (var product in products)
                {
                    table.AddRow(product.Id, product.Name, product.Price, product.Unit, product.Residue, product.Type, product.Status);
                }
        ...
        private void Buy()
        {
            ...
            if (choose == "y" || choose == "Y")
            {
                ...
                foreach (var product in products)
                {
                    productRepo.Calculation(product);

                    summ += product.Price * (decimal)product.Residue;

                    historyRepo.Create(new History()
                    {
                        CustomerId = MainMenu.us.Id,
                        ProductId = product.Id,
                        Quantity = product.Residue
                    });
                }
